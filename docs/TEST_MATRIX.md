...
| `API-002` | Build-time generation | `make verify-generate`, `.github/workflows/verify-generate.yml` | Partial | Verification workflow exists but is manual-only (`workflow_dispatch`) and not enforced on pull requests. |
